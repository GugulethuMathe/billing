    ...
    <div class="container">
        ...
        <article>
            <address>
                <?php
                $tenant = "";
                foreach ($records['detailed'] as $row) {
                    $tenant = $row->tenant;
                }

                $db = \Config\Database::connect();
                $query   = $db->query("SELECT * FROM `net_users` WHERE tenant_code='$tenant'");
                ...
                <p></p>
                <h3  style="margin-bottom:6px;">To</h3>
                <h3 style="margin-bottom:6px;"><?php echo $organisation?></h3>
            ...
            <table class="inventory">
                <thead>
                    <tr>
                        ...
                        <th>Setup Fee</th>
                        <th>Monthly</th>
                        <th>Minutes Charged</th>
                    </tr>
                </thead>
                <?php $sum = 0; ?>
                <?php foreach ($records['summary'] as $row) { ?>
                    <tbody>
                        <tr>
                            <?php
                            $widget = str_replace(array('<', '>'), '', $row->clid);
                            $widget_id = str_replace(array('"', '""'), '', $widget);
                            ?>
                            <td><span>Extension</span></td>
                            <td><span><?php echo $widget_id; ?></span></td>
                            <td><span>0</span></td>
                            <td><span>0</span></td>
                            <td><span><?php echo $row->minute_charged; ?></td>
                            <?php $string = $row->minute_charged;
                            $float_value = floatval($string);
                            $sum += $float_value; ?></span></td>
                        </tr>
                    </tbody>
                <?php } ?>
            </table>
            <table style="float:right;" class="meta">
                ...
                <tr>
                    ...
                    <td><span data-prefix>R</span>
                        <span><?php $totalfloat = number_format($sum, 2) + number_format($sum * 0.15, 2);
            ...
            <table id="page_break" class="inventory">
                <thead>
                    <tr>
                        <th><span contenteditable>Date</span></th>
                        <th><span contenteditable>Time</span></th>
                        <th><span contenteditable>Direction</span></th>
                        <th><span contenteditable>Caller</span></th>
                        <th><span contenteditable>Called</span></th>
                        <th><span contenteditable>Status</span></th>
                        <th><span contenteditable>Duration(sec)</span></th>
                        <th><span contenteditable>Cost</span></th>
                    </tr>
                </thead>
                <?php foreach ($records['detailed'] as $row) { ?>
                    <tbody>
                        <tr>
                            ...
                            <td><span contenteditable><?php
                                                        ...
                                                        $time = $row->calldate;
                                                        echo DateTime::createFromFormat("Y-m-d H:i:s", $time)->format("H:i:s");
                                                        ?></span></td>
                            <td><span contenteditable><?php
                                                        if (str_contains($row->lastdata, 'XL')) {
                                                            echo 'Incoming';
                                                        } else {
                                                            echo 'Outgoing';
                                                        }
                            ...
                            <td><span contenteditable>
                                    <?php $widget = str_replace(array('<', '>'), '', $row->clid);
                                    $widget_id = str_replace(array('"', '""'), '', $widget);
                                    echo $widget_id; ?></span></td>
                            <td><span contenteditable><?php echo $row->dst; ?></span></td>
                            <td><span contenteditable><?php echo $row->disposition; ?></td>
                            <td><span contenteditable><?php echo $row->duration; ?></td>
                            <td><span contenteditable><?php echo $row->billamount; ?></td>
                        </tr>
                    </tbody>
                <?php } ?>
            </table>
